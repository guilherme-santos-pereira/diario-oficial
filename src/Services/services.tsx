...
const services = {
    getFiles: async (page: string) => {
        return axios
            .get(`${PATH.base}/all-files/${page ? `?page=${page}` : ""}`, defaultHeaders)
            .then((data: any) => {
                return data;
            })
        ...
        return axios
            ...
            .then((data: any) => {
                if (body.username && body.password) {
                    ...
                    sessionStorage.setItem("credentials", btoa(`${body.username}:${body.password}`));
                }
                return data;
            })
            .catch((err: any) => console.log(err));
    ...
    getAllPosts: async (page: string) => {
        return axios
            .get(`${PATH.base}/all-posts/${page ? `?page=${page}` : ""}`, defaultHeaders)
            .then((data: any) => {
                return data;
            })
            .catch((err: any) => console.log(err));
    ...
    getPublic: async (body: any) => {
        ...
        const url = `${PATH.base}/search-files/${queryString.length > 0 ? "?" + queryString : ""}`;
        return axios
